...
def main():
    ...
    # 4. Create exports (if test file exists)
    export_test = Path(__file__).parent / "test_create_export.py"
    if export_test.exists():
        results["create_export"] = run_tests(
            "test_create_export.py::TestCreateExportIntegration",
            "Create Exports"
        )
    else:
        print(f"\n⏭️  Skipping test_create_export.py (not found)")
